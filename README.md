...
3. Run `.\setup-webui.ps1` once. It creates `webui\venv`, installs open-webui, and asks once via UAC to open inbound TCP 8080 for LAN access (`-SkipFirewall` skips the rule). Re-running is safe; a venv built with the wrong Python is rebuilt automatically.
