...
- Windows 10 (target 21H2), PowerShell 5.1 (built in). No Python, no nssm, no extra tools for the router itself; the optional Open WebUI chat needs Python 3.11/3.12 (installers bundled in [`files\`](files/README.md), see below).
- The optional Stable Diffusion WebUI image API also needs Python 3.11 or 3.12 (3.12 recommended: 3.11 no longer ships installers), Git for Windows, an NVIDIA GPU (CUDA-capable torch), and a model checkpoint (see below). Install them with `winget install Python.Python.3.12` / `winget install Git.Git`, or from the direct downloads: [python.org/downloads](https://www.python.org/downloads/) and [git-scm.com/download/win](https://git-scm.com/download/win). Bundled Python and Git installers are in [`files\`](files/README.md). winget is present on Windows 10 (1809+) via the Microsoft Store App Installer; if it's missing or stale, the direct installers work everywhere.
...
1. Install **Python 3.11 or 3.12** on the Windows host (`winget install Python.Python.3.12`, or from [python.org](https://www.python.org/downloads/)). Newer Pythons (3.13/3.14) are not supported by open-webui. The last 3.11 and 3.12 installers are also bundled in [`files\`](files/README.md) with verified SHA-256.
...
1. Install **Python 3.11 or 3.12** and **Git for Windows** (`winget install Python.Python.3.12`, `winget install Git.Git`), or download them directly: [Python](https://www.python.org/downloads/) and [Git for Windows](https://git-scm.com/download/win). Use 3.12 if you can: 3.11 no longer ships regular installers, and the WebUI rejects 3.13+. If you do want 3.11, it is the last official Windows installer. Install Python only from python.org (never a third-party repackaged build). The last 3.11 and 3.12 Python installers and the Git for Windows installer are also bundled in [`files\`](files/README.md) with verified SHA-256, handy for offline installs. All Python activity stays inside `sd\\venv`.
2. Make sure `setup-stablediffusion.ps1` is in the install folder (it is generated by every setup run; an EXE from v1.0.5 onward installs it).
3. Run `.\\setup-stablediffusion.ps1` once. It clones stable-diffusion-webui into `sd\\`, creates `sd\\venv` and installs its requirements, and asks once via UAC to open inbound TCP 7860 for LAN access (`-SkipFirewall` skips the rule). Re-running is safe; a venv built with the wrong Python is rebuilt automatically. All Python and pip activity stays strictly inside `sd\\venv`, so your system Python is never touched.
4. Get a checkpoint. Run `.\\Download-Model.ps1` to fetch Stability AI's SD 1.5 (open license, about 2 GB) into `sd\\stable-diffusion-webui\\models\\Stable-diffusion\\`. Or drop any `.safetensors`/`.ckpt` file there yourself (from [Hugging Face](https://huggingface.co/) or [Civitai](https://civitai.com/)). Pass `-Url <direct-file-link>` to Download-Model.ps1 to fetch a different model; without a checkpoint the service starts but `txt2img` returns an error.
