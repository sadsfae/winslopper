...
2. Ensure `setup-stablediffusion.ps1` is in the install folder. The release EXE installs it automatically — on that path you never run `setup-llama-server.ps1`. For a manual/source install (git clone), run `.\setup-llama-server.ps1` first; it writes `setup-stablediffusion.ps1`, `Download-Model.ps1` and the `stablediffusion` launcher.
