...
- The optional Stable Diffusion WebUI image API also needs Python 3.11 or 3.12 (3.12 recommended: 3.11 no longer ships installers), Git for Windows, an NVIDIA GPU (CUDA-capable torch), and a model checkpoint (see below). Install them with `winget install Python.Python.3.12` / `winget install Git.Git`, or from the direct downloads: [python.org/downloads](https://www.python.org/downloads/) and [git-scm.com/download/win](https://git-scm.com/download/win). Bundled Python installers are in [`files\`](files/README.md). winget is present on Windows 10 (1809+) via the Microsoft Store App Installer; if it's missing or stale, the direct installers work everywhere.
...
1. Install **Python 3.11 or 3.12** and **Git for Windows** (`winget install Python.Python.3.12`, `winget install Git.Git`), or download them directly: [Python](https://www.python.org/downloads/) and [Git for Windows](https://git-scm.com/download/win). Use 3.12 if you can: 3.11 no longer ships regular installers, and the WebUI rejects 3.13+. If you do want 3.11, it is the last official Windows installer. Install Python only from python.org (never a third-party repackaged build). The last 3.11 and 3.12 installers are also bundled in [`files\`](files/README.md) with verified SHA-256, handy for offline installs. All Python activity stays inside `sd\\venv`.
