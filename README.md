...
- The optional Stable Diffusion WebUI image API also needs Python 3.10 or 3.11, Git for Windows, an NVIDIA GPU (CUDA-capable torch), and a model checkpoint (see below).
...
1. Install **Python 3.10 or 3.11** and **Git for Windows** (`winget install Python.Python.3.11`, `winget install Git.Git`). AUTOMATIC1111 is tested on Python 3.10, and its pinned CUDA torch (2.1.2) has no 3.12+ wheels, so 3.12 will fail at first start.
...
4. Drop a checkpoint (`.safetensors` or `.ckpt`) into `sd\\stable-diffusion-webui\\models\\Stable-diffusion\\`. Without one the service starts but returns an error on generate.
