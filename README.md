...
2. Generate the SD scripts first. `setup-stablediffusion.ps1` (plus `Download-Model.ps1` and the `stablediffusion` launcher) is written by `setup-llama-server.ps1`, so on a fresh clone run that first: `.\setup-llama-server.ps1`. (An EXE from v1.0.15 onward installs these too.)
